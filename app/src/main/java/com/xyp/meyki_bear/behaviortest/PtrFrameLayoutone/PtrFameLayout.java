 ...
 * 类描述：
 * 创建人：xyp
 * 创建时间：2017/6/15 10:14
 * 修改人：meyki-bear
 * 修改时间：2017/6/15 10:14
 * 修改备注：
 */

public class PtrFameLayout extends PtrFrameLayout implements NestedScrollingChild, NestedScrollingParent {
    ...
    // config
    private PtrFrameLayout.Mode mMode = PtrFrameLayout.Mode.BOTH;
    ...
    public PtrFameLayout(Context context) {
        this(context, null);
    }

    public PtrFameLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public PtrFameLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        if (arr != null) {
            ...
            mFooterId = arr.getResourceId(in.srain.cube.views.ptr.R.styleable.PtrFrameLayout_ptr_footer, mFooterId);

            mPtrIndicator.setResistanceHeader(arr.getFloat(in.srain.cube.views.ptr.R.styleable.PtrFrameLayout_ptr_resistance, mPtrIndicator.getResistanceHeader()));
            mPtrIndicator.setResistanceFooter(arr.getFloat(in.srain.cube.views.ptr.R.styleable.PtrFrameLayout_ptr_resistance, mPtrIndicator.getResistanceFooter()));
            mPtrIndicator.setResistanceHeader(arr.getFloat(in.srain.cube.views.ptr.R.styleable.PtrFrameLayout_ptr_resistance_header, mPtrIndicator.getResistanceHeader()));
            mPtrIndicator.setResistanceFooter(arr.getFloat(in.srain.cube.views.ptr.R.styleable.PtrFrameLayout_ptr_resistance_footer, mPtrIndicator.getResistanceFooter()));

            mDurationToBackHeader = arr.getInt(in.srain.cube.views.ptr.R.styleable.PtrFrameLayout_ptr_duration_to_back_refresh, mDurationToCloseHeader);
            mDurationToBackFooter = arr.getInt(in.srain.cube.views.ptr.R.styleable.PtrFrameLayout_ptr_duration_to_back_refresh, mDurationToCloseHeader);
            mDurationToBackHeader = arr.getInt(in.srain.cube.views.ptr.R.styleable.PtrFrameLayout_ptr_duration_to_back_header, mDurationToCloseHeader);
            mDurationToBackFooter = arr.getInt(in.srain.cube.views.ptr.R.styleable.PtrFrameLayout_ptr_duration_to_back_footer, mDurationToCloseHeader);

            mDurationToCloseHeader = arr.getInt(in.srain.cube.views.ptr.R.styleable.PtrFrameLayout_ptr_duration_to_close_either, mDurationToCloseHeader);
            mDurationToCloseFooter = arr.getInt(in.srain.cube.views.ptr.R.styleable.PtrFrameLayout_ptr_duration_to_close_either, mDurationToCloseFooter);
            ...
            mDurationToCloseFooter = arr.getInt(in.srain.cube.views.ptr.R.styleable.PtrFrameLayout_ptr_duration_to_close_footer, mDurationToCloseFooter);
            ...
            mMode = getModeFromIndex(arr.getInt(in.srain.cube.views.ptr.R.styleable.PtrFrameLayout_ptr_mode, 4));
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mContent != null) {
            ...
            if (getLayoutParams().height == PtrFrameLayout.LayoutParams.WRAP_CONTENT) {
    ...
    public boolean dispatchTouchEvent(MotionEvent e) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_DOWN:
                ...
                vtev.offsetLocation(offsetInWindow[0], offsetInWindow[1]);
                //重新赋值偏移
                mLastMoveEvent = e;
            ...
            case MotionEvent.ACTION_MOVE:
                //修正偏移量
                int moveX = (int) (e.getX() - lastX);
                int moveY = (int) (e.getY() - lastY);
                lastX = e.getX();
                lastY = e.getY();
                ...
                if (mPreventForHorizontal) {
                    ...
                    boolean b = dispatchTouchEventSupper(e);
                    //把事件放给子View后计算一下屏幕偏移
                    offsetInWindow[0] = offsetInWindow[1] = 0;
                    getLocationInWindow(offsetInWindow);
                ...
                if (!canMoveUp && !canMoveDown) {
                    ...
                    if (moveDown && !canHeaderMoveDown) {
                        ...
                        vtev.offsetLocation(offsetInWindow[0], offsetInWindow[1]);
                        //重新赋值偏移
                        mPtrIndicator.onMove(e.getX(), e.getY() - offsetInWindow[1]);
                        return b;
                    }
                    if (moveUp && !canFooterMoveUp) {
                        ...
                        vtev.offsetLocation(offsetInWindow[0], offsetInWindow[1]);
                        //重新赋值偏移
                        mPtrIndicator.onMove(e.getX() - offsetInWindow[0], e.getY() - offsetInWindow[1]);
                        return b;
                    }

                    // should show up header
                    if (moveDown) { //可以滑动头尾布局的时候，先把事件交给别的控件消费
                        //pointf是上一次滑动的坐标点，如果这个事件被消费了，是否可以认为是本次没有移动，如果没有消费完，是否可以认为本次的触摸点是剩下的距离
                        if (moveY == 0 || dispatchNestedPreScroll(-moveX, -moveY, mScrollConsumed, mScrollOffset)) {
                            //重新赋值偏移
                            vtev.offsetLocation(mScrollOffset[0], mScrollOffset[1]);
                            lastX = e.getX() - mScrollOffset[0];
                            lastY = e.getY() - mScrollOffset[1];
                            mPtrIndicator.onMove(e.getX() - offsetInWindow[0], e.getY() - offsetInWindow[1]);
                        } else {
                    ...
                    if (moveUp) {
                        //上滑先不处理，距离计算有问题，滑动比较困难，初步怀疑是下拉刷新控件的阻尼系数导致的
                        //不知什么情况，MOVEY可能为0，如果MOVEY为0则不会受理这个滑动事件，倒置滑动情况进入第二种可能，头布局被滑出
                        if (moveY == 0 ||dispatchNestedPreScroll(-moveX, -moveY, mScrollConsumed, mScrollOffset)) {
                            //pointf是上一次滑动的坐标点，如果这个事件被消费了，是否可以认为是本次没有移动，如果没有消费完，是否可以认为本次的触摸点是剩下的距离
                            vtev.offsetLocation(mScrollOffset[0], mScrollOffset[1]);
                            lastX = e.getX() - mScrollOffset[0];
    ...
    public void setFooterView(View footer) {
        ...
        if (lp == null) {
            lp = new PtrFrameLayout.LayoutParams(-1, -2);
    ...
    protected ViewGroup.LayoutParams generateDefaultLayoutParams() {
        return new PtrFrameLayout.LayoutParams(PtrFrameLayout.LayoutParams.MATCH_PARENT, PtrFrameLayout.LayoutParams.MATCH_PARENT);
    }
    ...
    public ViewGroup.LayoutParams generateLayoutParams(AttributeSet attrs) {
        return new PtrFrameLayout.LayoutParams(getContext(), attrs);
    }
